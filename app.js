...
function loadChat() {
  ...
  // NOTE: Do NOT use orderByChild('createdAt') here — it requires a Firebase
  // index rule (.indexOn: ["createdAt"]) on community/chat. Instead, read the
  // last 60 push-key ordered children and sort by createdAt client-side.
  ...
  _chatUnsubscribe = _fb.onValue(chatQ, snap => {
    ...
    // Sort by createdAt ascending (Firebase push keys are already roughly ordered
    // but explicit sort ensures correctness even if client clocks differ)
        ...
        <div class="chat-msg ${isMe ? 'chat-msg-me' : ''}">
          <div class="chat-msg-inner">
            ...
            <div class="chat-msg-body">
              ${!isMe ? `<div class="chat-msg-sender">${escapeHtml(msg.name || '')}</div>` : ''}
    ...
    console.error('[FUTA] Chat listener error:', err);
    messagesEl.innerHTML = `<p style="text-align:center;color:var(--text-dim);padding:2rem;font-size:.85rem">Could not load chat. Please refresh.</p>`;
  });
...
async function loadStudentsList() {
  ...
  try {
    // Get follows once (rarely changes mid-session)
    ...
    // Use real-time listener so newly registered students appear without reload
    _studentsUnsubscribe = _fb.onValue(_fb.ref(_db, 'users'), usersSnap => {
      const container = document.getElementById('students-list-content');
      if (!container) return;

      if (!usersSnap.exists()) {
        container.innerHTML = `<p style="color:var(--text-dim);padding:1rem">No students found.</p>`;
        return;
      }
          ...
          ${users.map(u => {
            ...
            return `
              <div class="student-card" id="student-card-${u.uid}">
                ...
                <div class="student-card-meta">${escapeHtml(u.level || '')} · ${escapeHtml(u.matric || '')}</div>
      ...
      console.error('[FUTA] Students listener error:', err);
      const container = document.getElementById('students-list-content');
      if (container) container.innerHTML = `<p style="color:var(--text-dim);padding:1rem">Could not load students. Please refresh.</p>`;
    });

    // Detach listener when community tab switches away
    const origCleanup = window._cleanupChat;
    window._cleanupStudents = () => {
      if (_studentsUnsubscribe) { _studentsUnsubscribe(); _studentsUnsubscribe = null; }
    };
  } catch(e) {
    el.innerHTML = `<p style="color:var(--text-dim);padding:1rem">Could not load students.</p>`;
    console.error('[FUTA] loadStudentsList error:', e);
  }
}
