...
function renderHome() {
  ...
  if (_currentUser) {
    ...
    const firstName = _userProfile ? _userProfile.name.split(' ')[0] : (_currentUser.email || '').split('@')[0];
    const hour      = new Date().getHours();
    const timeGreet = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening';
    quickLinks.innerHTML = `
      <div class="home-greeting-banner">
        <span class="home-greeting-wave">👋</span>
        <span>${timeGreet}, <strong>${firstName}</strong>! Welcome back.</span>
      </div>
