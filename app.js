...
/* ─────────────────────────────────────────────────────────────────
   RENDER QUESTION — supports both multiple-choice and fill-in-the-blank
   
   Multiple choice format (existing):
     { text, options: [...], correct: 0, explanation }
   
   Fill-in-the-blank format (new):
     { type: "fill", text, correctAnswer: "answer", explanation }
     - correctAnswer comparison is case-insensitive and trims whitespace
     - You may also provide: acceptedAnswers: ["ans1", "ans2"] for alternate spellings
───────────────────────────────────────────────────────────────── */
function renderCBTQuestion() {
  ...
  const questionBody = isFill
    ? renderFillQuestion(q, current, total)
    : renderMCQQuestion(q, current, total);
  ...
  /* For fill-in questions, attach Enter-key listener on the input */
  if (isFill) {
...
function renderFillQuestion(q) {
  return `
    <div class="cbt-fill-wrap">
      <input
        type="text"
        id="fill-input"
        class="cbt-fill-input"
        placeholder="Type your answer here…"
        autocomplete="off"
        spellcheck="false"
      />
      <button class="btn btn-primary cbt-fill-submit" onclick="submitFill()">Submit Answer</button>
    </div>`;
}
...
/* ─── Fill-in-the-blank answer handler ─── */
function submitFill() {
  if (cbtState.answered) return;
  const inp = document.getElementById('fill-input');
  if (!inp) return;
  ...
  if (!userAnswer) {
    inp.classList.add('cbt-fill-empty');
    setTimeout(() => inp.classList.remove('cbt-fill-empty'), 600);
    return;
  }
  cbtState.answered = true;
  ...
  /* Build list of accepted answers (case-insensitive, trimmed) */
  const accepted = [];
  if (q.correctAnswer) accepted.push(q.correctAnswer.trim().toLowerCase());
  ...
  /* Style the input */
  inp.disabled = true;
  inp.classList.add(isCorrect ? 'fill-correct' : 'fill-wrong');

  /* Hide submit button */
  const submitBtn = document.querySelector('.cbt-fill-submit');
