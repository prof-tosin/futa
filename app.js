...
document.getElementById(‘hero-stats’).innerHTML = `<div class="stat"><div class="stat-num">${PORTAL_DATA.levels.length}</div><div class="stat-label">Levels</div></div> <div class="stat"><div class="stat-num">${totalCourses}</div><div class="stat-label">Courses</div></div> <div class="stat"><div class="stat-num">${totalChapters}</div><div class="stat-label">Chapters</div></div> <div class="stat"><div class="stat-num">${totalQ}+</div><div class="stat-label">Practice Questions</div></div>`;
...
return `<div class="level-card ${hasCourses ? '' : 'empty'}" onclick="${hasCourses ?`navigate(‘level’,’${lvl.id}’)`: 'void(0)'}"> <div class="level-badge">${lvl.name}</div> <div class="level-name">${lvl.name}</div> <div class="level-full">${lvl.fullName}</div> <div class="level-footer"> ${hasCourses ?`<span class="level-count">${totalCrs} course${totalCrs !== 1 ? ‘s’ : ‘’}${semCount ? ’ · ’ + semCount : ‘’}</span>
<span class="level-arrow">→</span>`:`<span class="coming-soon-tag">Coming Soon</span>`} </div> </div>`;
}).join(’’);
}
...
html += `<div class="course-card ${hasChapters ? '' : 'empty'}" onclick="${hasChapters ?`navigate(‘course’,’${escQ(levelId)}’,’${escQ(course.code)}’)`: 'void(0)'}"> <div class="course-code">${course.code}</div> <div class="course-title">${course.title}</div> <div class="course-meta"> ${hasChapters ?`<span class="chapter-count">📖 ${course.chapters.length} chapter${course.chapters.length !== 1 ? ‘s’ : ‘’}</span>
<span class="course-arrow">→</span>`:`<span class="coming-soon-tag">Coming soon</span>`} </div> </div>`;
});
html += `</div>`;
}
html += `</div>`;
...
${ch.keyPoints && ch.keyPoints.length ? `<div class="ch-keypoints"> <div class="ch-keypoints-title">Key Points</div> <ul>${ch.keyPoints.map(kp =>`<li>${kp}</li>`).join('')}</ul> </div>` : ‘’}
...
document.getElementById(‘cbt-body’).innerHTML = `<div class="cbt-progress"><div class="cbt-progress-bar" style="width:${pct}%"></div></div> <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;"> <div class="cbt-q-count">Question ${current + 1} of ${total}</div> ${cbtState.timeLimit > 0 ?`<div id="cbt-timer" style="font-family:'JetBrains Mono',monospace;font-size:0.85rem;font-weight:600;color:var(--gold);background:rgba(201,146,26,0.1);border:1px solid rgba(201,146,26,0.3);padding:4px 12px;border-radius:4px;">–:–</div>`: ''} </div> <div class="cbt-question">${q.text}</div> ${bodyHTML} <div class="cbt-feedback" id="cbt-feedback"></div> ${navHTML} <div class="cbt-nav"> ${isTest && current > 0 ?`<button class="btn btn-ghost cbt-prev-btn" onclick="prevCBTQuestion()">← Previous</button>`: '<span></span>'} <button class="btn btn-primary" id="cbt-next-btn" ${!isTest ? 'style="display:none"' : ''} onclick="${isTest ? 'saveAndNext()' : 'nextCBTQuestion()'}"> ${isTest ? (current + 1 >= total ? '✔ Submit Test' : 'Save →') : (current + 1 >= total ? 'See Results' : 'Next →')} </button> </div>`;
...
return `<div class="cbt-options"> ${q.options.map((opt, i) =>`
...
/* ── True / False renderer ── */
function renderTF() {
return ` <div class="cbt-options"> <button class="cbt-option" id="opt-0" onclick="answerTF(true)">A. True</button> <button class="cbt-option" id="opt-1" onclick="answerTF(false)">B. False</button> </div>`;
}

/* ── Fill-in-the-blank renderer ── */
function renderFITB(q) {
const isTest = cbtState.isTest;
return `<div class="cbt-fitb-wrap"> <input type="text" id="fitb-input" class="cbt-fitb-input" placeholder="${isTest ? 'Type your answer here…' : 'Type your answer and press Enter…'}" autocomplete="off" spellcheck="false" /> ${!isTest ?`<button class="btn btn-primary cbt-fitb-btn" onclick="submitFITB()">Submit</button>` : ''} </div>`;
}
...
return ` <div class="cbt-qnav-wrap"> <div class="cbt-qnav-label"> <span class="qnav-legend"><span class="qnav-dot answered"></span>Answered</span> <span class="qnav-legend"><span class="qnav-dot skipped"></span>Skipped</span> <span class="qnav-legend"><span class="qnav-dot future"></span>Remaining</span> </div> <div class="cbt-qnav" id="cbt-qnav">${boxes}</div> </div>`;
}
...
document.getElementById(‘cbt-body’).innerHTML = `<div class="cbt-score-screen"> ${timeMsg} <div class="score-ring" style="--pct:${pct * 3.6}deg"> <div class="score-num">${pct}%</div> </div> <div class="score-label">${grade}</div> <div class="score-sub">You scored ${score} out of ${total} questions.</div> <div style="margin-top:26px;display:flex;gap:10px;justify-content:center;flex-wrap:wrap"> <button class="btn btn-primary" onclick="restartCBT()">🔄 Try Again</button> <button class="btn btn-ghost" onclick="closeCBT()">Close</button> </div> ${reviewBtn} </div>`;
}

function showTestReview() {
const { questions, userAnswers } = cbtState;
let html = `<div class="test-review"> <div class="test-review-header"> <span style="font-family:'Playfair Display',serif;font-size:1.1rem;font-weight:700;color:var(--text);">Test Review</span> <span style="font-size:0.75rem;color:var(--text-muted);font-family:'Lora',serif;font-style:italic;">All questions, your answers & explanations</span> </div>`;
